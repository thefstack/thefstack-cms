...
import BlogScheduleManager from "./BlogScheduleManager"

export default function BlogManager() {
  const [blogs, setBlogs] = useState([])
  const [showViewPopup, setShowViewPopup] = useState(false)
  const [selectedBlog, setSelectedBlog] = useState(null)
  const [activeTab, setActiveTab] = useState("blogs") // "blogs" or "schedules"
  ...
  return (
    <div className={styles.blogManager}>
      ...
      <div className={styles.tabs}>
        ...
        <button
          className={`${styles.tabButton} ${activeTab === "schedules" ? styles.activeTab : ""}`}
            ...
            {Array.isArray(blogs) &&
              blogs.map((blog) => (
                <li key={blog.id} className={styles.blogItem} onClick={() => handleView(blog)}>
                  {blog.thumbnail && (
                    <img src={blog.thumbnail || "/placeholder.svg"} alt={blog.title} className={styles.thumbnail} />
                  )}
                  <h3>{blog.title}</h3>
              ...
              <div className={styles.popupContent}>
                <div className={styles.popupContentData}>
                  {selectedBlog.thumbnail && (
                    <img
                      src={selectedBlog.thumbnail || "/placeholder.svg"}
                ...
                <div className={styles.buttonContainer}>
                  <button type="button" className={styles.button} onClick={() => setShowViewPopup(false)}>
                    Close
                  </button>
                  <button type="button" className={styles.button} onClick={() => handleDelete(selectedBlog._id)}>
                    Delete
                  </button>
                </div>
              </div>
